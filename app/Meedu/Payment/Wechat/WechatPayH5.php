...
class WechatPayH5 extends WechatPayBase
{
    public function create(string $orderNo, string $title, int $realAmount, array $extra = []): PaymentStatus
    {
        try {
            ...
            $createResult = Pay::wechat($this->getConfig())->wap($payOrderData);

            return new PaymentStatus(true, response()->json([
                'code' => 0,
                'message' => '',
                'data' => [
                    'redirect_url' => $createResult->getTargetUrl(),
                ],
            ]));
        } catch (\Exception $exception) {
            Log::error(__METHOD__ . '|微信H5支付订单创建失败,信息:' . $exception->getMessage());
